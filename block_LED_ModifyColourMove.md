...
> select a ``||logic.ifelse()||`` instruction
and dropping it inside the **if** instruction

> delect anoyhrt  ``||logic.if()||`` instruction
and dropping it inside the **else** part of the **if elese** instruction

>> this creates a *nested* set of *if* instruction


> select a ``||logic.compare(=)||`` ** = ** instruction 
>> these are at the bottom of the list shaped with angled ends 

>> drop this into the box the same shape at the top of the **if** instruction 
...
**white circle** at the top of the outer **if compare =** instruction 

> For the innner **if** compare = instructions 
do the same but put drop the **targetStart** variable red oval 
into their left hand **white circle** 
